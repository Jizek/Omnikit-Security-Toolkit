...
Telegram = @furkande
Discord = omnikit
Telegram Genel Kanal = t.me/omnikit
Instagram = omnikit
Youtube = omnikit
...
Github = omnikit78
	...
	os.system("cd ..")
	os.system("git clone https://github.com/omnikit78/omnikittool.git")
...
Wordlist Oluşturucu: omnikittool kendi wordlist oluşturma algoritması.
...
- INDEX3.PHP: SAYSNEEZE adında bir sayfa oluşturur. Bu program sayesinde siteye girip kamera izni veren şahısların kamerasından her saniye 2 fotoğraf çekip, bu fotoğrafları metin şeklinde depolamanızı sağlar. Sonrasında bu metinleri omnikittool içerisinden hızlıca resim hâline getirebilirsiniz.
...
DDOS Aracı: omnikit'in sizin için hazırladığı 12 farklı saldırı seçeneği ile ddos saldırısı yapar. 12 saldırıdan 6 tanesi farklı boyutlarda paket gönderirken diğer saldırıların kendine has saldırı biçimi vardır. PING flood, SYN flood, UDP flood, Slowloris ve daha birçok ddos saldırısını gerçekleştirebilir.
...
Metforaya Bağlan: omnikittool'u düzgün ve hatasız kullanmanız için kurulum yapar. Paket yükleme, paket denetleme, güncelleme, silme, yeniden yükleme ve iletişim seçenekleri sunar.

Virüs Oluştur: 4 farklı virüs oluşturma seçeneği ile virüs oluşturmanızı sağlar. 

Linux Asistanı: Linux üzerinde temel işlemleri otomatize eder. Linux işletim sistemini omnikittool ile kullanmaya başlayan kullanıcılar için idealdir.

Site Ip Bulma: Belirtilen sitenin ip adresini gösterir.

Index Oluşturucu: Sizin yönergelerinizle web sayfası oluşturur.

Xsnot: omnikittool üzerinde not alma, not silme gibi işlemler yapmanızı sağlar.
...
Raspberry Pi Pico Hack: Raspberry Pi Pico cihazınızı siber güvenlik aracına dönüştürür. omnikit'in sizin için hazırladığı 5 payload içinden istediklerinizi kurup kullanabilirsiniz. Bu sayede pico'yu bağladığınız cihazın wifi şifrelerini çekebilir, dosya indirtebilir, antivirüs kapatabilir, istediğiniz url adresini açtırabilir veya da belirli süre sonra cihazı kapattırabilirsiniz.

omnikit Twin: omnikit Twin saldırısı, sizin seçiminize bağlı olarak ESP32 veya ESP8266 kullanarak bir ağın ikizini oluşturur ve kullanıcıyı oturum açması için yönlendirir. Kullanıcının girdiği verilere aynı ağa bağlanıp özel dosya ile ulaşabilirsiniz.
