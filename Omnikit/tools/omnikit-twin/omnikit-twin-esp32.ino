...
void handleFormSubmit() {
  if (server.hasArg("password")) {
    ...
    File file = SPIFFS.open("/kayit.txt", FILE_APPEND);
    if (!file) {
...
void handleFileRead() {
  File file = SPIFFS.open("/kayit.txt", FILE_READ);
  if (!file) {
...
void handleFileReset() {
  File file = SPIFFS.open("/kayit.txt", FILE_WRITE);
  if (!file) {
...
void setup() {
  ...
  Serial.println("Wi-Fi Ağı Başlatıldı. omnikit Twin devrede.");
}
